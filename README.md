...
## 6. Aggiornamento: selettore in alto e rigenerazione codici (22 settembre)

**Per un database Supabase GIA' inizializzato**: NON rieseguire `backend/setup.sql` e NON ricreare l'account regia.

1. Prima di utilizzare i nuovi pulsanti, apri **Supabase → SQL Editor → New query**, incolla il contenuto integrale di `backend/upgrade-2026-09-22.sql` e premi **Run**. Questa migrazione aggiunge soltanto una funzione per rigenerare i codici: **non cancella** squadre, cronometri o risultati. Se stai installando su un database totalmente nuovo, esegui prima `backend/setup.sql`, crea il codice regia e poi applica questa migrazione.
2. Su GitHub, nel repository già pubblicato, **aggiorna solo** `index.html`, `app.js`, `assets/live.css` e carica il nuovo `backend/upgrade-2026-09-22.sql`. Non sostituire `site-config.js` con un file vuoto e non reinizializzare Supabase. Puoi aggiornare anche `README.md` per mantenere queste istruzioni.
3. Attendi che GitHub Pages pubblichi la nuova versione, poi apri il sito e premi `Ctrl+F5` per ricaricare i file aggiornati. Su smartphone può essere necessario svuotare la cache del sito o aprirlo in una nuova scheda privata.
4. Il selettore **Squadra** è adesso nella barra superiore sempre visibile. Regia e pubblico possono cambiare squadra; partecipanti e commissari vedono soltanto la squadra associata al loro accesso.
5. Accedi come regia, scorri a **Gestione codici di accesso** e scegli per la squadra interessata **Rigenera codice squadra** o **Rigenera codice commissario**. Conferma l'operazione, copia il nuovo codice mostrato e consegnalo in privato. Il vecchio codice **e le sessioni già aperte con quel ruolo** vengono revocati. Non vengono toccati risultati, squadre o altre credenziali. Non inserire mai i codici nelle issue GitHub o in screenshot pubblici.

**Nota:** dal database non è possibile ricostruire il vecchio codice, perché se ne conserva solo l'hash. La regia può generarne uno nuovo. La nuova funzionalità richiede che lo SQL qui sopra sia stato applicato con successo: pubblicare i file statici senza migrare il database produce un errore al clic sul pulsante.
