...
let displayedCodes=null, editorDraft=null, lastEditorStage=null, lastErrorTime=0;
...
async function logout(){if(gpsWatch!==null)await stopGps();token=null;sessionStorage.removeItem(TOKEN_KEY);selected=null;stage=1;displayedCodes=null;await refresh();toast('Hai effettuato l’uscita.')}
...
async function handleClick(e){const b=e.target.closest('button');if(!b)return;
 ...
 if(b.id==='add-team'){const name=$('new-team').value.trim();if(name.length<2){toast('Inserisci il nome della squadra.');return;}
   ...
   if(data){displayedCodes={name,teamCode:data.teamCode,commissionerCode:data.commissionerCode};editorDraft=null;lastEditorStage=null;render();toast('Squadra registrata. Conserva i codici mostrati in regia.')}return;}
